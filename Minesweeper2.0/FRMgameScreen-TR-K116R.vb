Public Class FRMgameScreen
    ...
    Public boxSize As New Size(40, 40)
    Public boxLoc As New Point(10, 10)
    ...
    Sub updateObList(x As Integer, y As Integer, c As Color, Optional show As Boolean = True)
        obList(x, y).panel.BackColor = c

        obList(x, y).label.BackColor = c

        If show Then
            obList(x, y).label.Visible = True

            obList(x, y).label.BringToFront()
        End If

    End Sub

    Sub isGreen(value As Point)
        ' not completed
        For x = -1 To 1
            For y = -1 To 1
                ...
                Try
                    If board(value.X + x, value.Y + y) = 0 And obList(value.X + x, value.Y + y).panel.BackColor <> backC Then
                        updateObList(value.X + x, value.Y + y, Color.Green)
                        isGreen(value:=New Point(value.X + x, value.Y + y))
                    End If
                Catch ex As Exception
    ...
    Sub boxClick(sender As Object, e As MouseEventArgs)
        ...
        Else
            ...
            If obList(tagX, tagY).panel.BackColor = backC Then
                updateObList(tagX, tagY, Color.Yellow, False)
            Else
                updateObList(tagX, tagY, backC, False)
            End If
    ...
    Sub checkPos(ByVal x As Integer, ByVal y As Integer, leftClick As Boolean)
        ...
        For tx = 0 To xTot
            For ty = 0 To yTot
                ...
                End If
                If obList(tx, ty).panel.BackColor = backC Then
                    winnable = False
                End If
            Next
        Next
